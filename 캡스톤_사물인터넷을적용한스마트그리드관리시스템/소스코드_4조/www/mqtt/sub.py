...
class Electricity(object):
	...
	def __init__(self, data):
		self.current = abs(float(data[0]))
		self.volt = abs(float(data[1]))
		self.on = abs(float(data[2]))
		if (len(data) > 3):
			self.flag = abs(float(data[3]))
	def toCSV(self):
		data = '{}, {}, {}'.format(self.current, self.volt, self.on)
		try:
			data = data+", {}".format(self.flag)
		except:
			pass
		return data
...
if topic == 'solar':
	...
	topic = 'house/solar/gen/#'
	client.on_message = on_message_battery
elif topic == 'device':
	topic = 'house/device/#'
	client.on_message = on_message_device
elif topic == 'consumption':
	topic= 'house/device/consumption/#'
	client.on_message = on_message_battery

client.connect("203.250.148.23", 1883, 60)

conn = mysql.connect(host="203.250.148.23", user="root", password="ngn787178", db="smartHome", charset="utf8")
